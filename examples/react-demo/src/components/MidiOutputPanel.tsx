...
interface MidiOutputPanelProps {
  onSendMessage: (message: Uint8Array) => void
}

export const MidiOutputPanel: React.FC<MidiOutputPanelProps> = ({ onSendMessage }) => {
  ...
  const sendPolyphonicAftertouch = () => {
    const message = buildPolyphonicAftertouch(channel, polyAftertouchNote, pressure)
    onSendMessage(message)
  }
    ...
    <div className="midi-output-panel">
      ...
      <div className="output-section">
        ...
        <div className="button-group">
          <button onClick={sendNoteOn} className="send-btn note-on">Note On</button>
          <button onClick={sendNoteOff} className="send-btn note-off">Note Off</button>
        </div>
      </div>
      ...
      <div className="output-section">
        ...
        <div className="control-group">
          <label>
            Controller: 
            <select value={controller} onChange={(e) => setController(Number(e.target.value))}>
        ...
        <div className="button-group">
          <button onClick={sendControlChange} className="send-btn control-change">Send CC</button>
        </div>
      </div>
      ...
      <div className="output-section">
        ...
        <div className="button-group">
          <button onClick={sendProgramChange} className="send-btn program-change">Send PC</button>
        </div>
      </div>
      ...
      <div className="output-section">
        ...
        <div className="control-group">
          <label>
            ...
            />
            <span>{pitchBendValue} ({pitchBendValue === 8192 ? 'Center' : pitchBendValue > 8192 ? '+' + (pitchBendValue - 8192) : '-' + (8192 - pitchBendValue)})</span>
          </label>
        </div>
        <div className="button-group">
          <button onClick={sendPitchBend} className="send-btn pitch-bend">Send Pitch Bend</button>
        </div>
      </div>
      ...
      <div className="output-section">
        ...
        <div className="control-group">
          <label>
            Polyphonic Note: 
            <select value={polyAftertouchNote} onChange={(e) => setPolyAftertouchNote(Number(e.target.value))}>
              {generateNoteOptions()}
            </select>
          </label>
          <label>
        ...
        <div className="button-group">
          <button onClick={sendChannelAftertouch} className="send-btn channel-aftertouch">Channel AT</button>
          <button onClick={sendPolyphonicAftertouch} className="send-btn polyphonic-aftertouch">Poly AT</button>
        </div>
      </div>
    </div>
  )
